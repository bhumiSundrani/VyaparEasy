...
const fetchSale = async (id: string): Promise<SaleFormData | null> => {
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/sales/${id}`, {
      method: 'GET',
      headers: {
...
export default async function Page({ params }: PageProps) {
  ...
  let sale: SaleFormData | null = null;

  if (id !== "new") {
    sale = await fetchSale(id);
    if (!sale) return notFound(); // show 404 if purchase doesn't exist
  }
