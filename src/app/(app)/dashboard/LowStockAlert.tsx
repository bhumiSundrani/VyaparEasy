...
const LowStockAlert = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
            const res = await axios.get("/api/dashboard/low-stock-alerts");
      ...
      <table className="min-w-full table-auto text-sm rounded-xl">
        ...
        <tbody>
          {products?.map((product, index) => (
            ...
            <td className="px-4 py-3 relative rounded-xl my-2 overflow-hidden inline-block bg-gray-100 border border-gray-200 shadow-sm">
                <img
                alt={product.name}
                src={product.imageUrl || "/product-icon-png-19.jpg"}
                className="object-cover h-10 transition-transform hover:scale-105" 
                onError={(e) => {
